...
class DialogManager: IDialogManager {
    ...
    override fun showTorrentDeletedDialog(fragmentManager: FragmentManager, torrentName: String, torrentHash: String) {
        val ft = fragmentManager.beginTransaction()
        val prev = fragmentManager.findFragmentByTag(TAG_DIALOG)
        if (prev != null) {
            ft.remove(prev)
        ...
        // Create and show the dialog.
        val newFragment = TorrentDeletedDialog.newInstance(torrentName, torrentHash)
        newFragment.show(ft, TAG_DIALOG)
    }
}